<?php
  ...
  /*function getContent($sql) {
    //$sql = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '1'";
    $conn = mysqli_connect("173.180.133.176", "root", "superpassword", "hi-tec");
  ...
  function getContent($sql) {
    ...
    if($result->num_rows > 0) {
      ...
      while($row = $result->fetch_assoc()) {
        ...
        $content .= "<td>";
        $content .= "<strong>Product: <span style=\"color:blue;\">" . $productname . "</span></strong>";
        $content .= "</br><strong>Manufacturer: <span style=\"color:blue;\">" . $manufacturer . "</span></strong><br/>";
        if($stock > 0) {
          $content .= "</br><span style=\"font-size:16px;\" class=\"label label-success\">In Stock</span>";
          /*$content .= "<div style=\"width:50%;\" class=\"alert alert-success\">
      			<center><strong>In Stock</strong></center>
      		</div>";*/
        }else{
          $content .= "</br><span style=\"font-size:16px;\" class=\"label label-danger\">Out of Stock</span>";
  ...
  if(isset($_GET["cat"])) {
    ...
    if(is_numeric($cat)) {
  	if($cat == 1) {
  	  $catName = "";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id";
      $content = getContent($test);
      }else if($cat == 2) { // RAM
  	  $catName = "RAM";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '1'";
      $content = getContent($test);
      }else if($cat == 3) { //Motherboard
        $catName = "Motherboard";
        $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '2'";
        $content = getContent($test);
      }else if($cat == 4) { //GPU
  	  $catName = "GPU";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '3'";
      $content = getContent($test);
  	}else if($cat == 5) { //CPU
        $catName = "CPU";
        $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '4'";
        $content = getContent($test);
      }else if($cat == 6) { //Storage
        $catName = "Storage";
        $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '5'";
        $content = getContent($test);
      }else if($cat == 7) { //PSU
        $catName = "PSU";
        $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '6'";
        $content = getContent($test);
      }else if($cat == 8) { //Displays
  	  $catName = "Displays";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '7'";
      $content = getContent($test);
  	}else if($cat == 9) { //Keyboards
  	  $catName = "Keyboards";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '8'";
      $content = getContent($test);
  	}else if($cat == 10) { //Mouse
  	  $catName = "Mouse";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '9'";
      $content = getContent($test);
  	}else if($cat == 11) { //Desktop
  	  $catName = "Desktop";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '10'";
      $content = getContent($test);
  	}else if($cat == 12) { //Accessories
  	  $catName = "Accessories";
      $test = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where category_id = '11'";
      $content = getContent($test);
  	}else {
  	  //invalid category
  	  $content = "<div class=\"alert alert-danger\" role=\"alert\">
  ...
  if(isset($_GET["search"])) {
    ...
    if(strlen($search) > 0) {
      $sql = "SELECT product_name,product_img_path,product_stock,manufacturer_name from product inner join manufacturer on product.manufacturer_id = manufacturer.manufacturer_id where product_name LIKE '%" . $search . "%'";
